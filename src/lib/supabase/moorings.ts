...
export async function updateMooring(
  ...
  formData: FormData
): Promise<FormState> { // Update return type
  ...
  const { data: existingMooring, error: fetchError } = await supabase
    .from('moorings')
    .select('owner_id')
    .eq('id', mooringId)
  ...
  if (!existingMooring || existingMooring.owner_id !== user.id) {
    ...
    return { message: 'Authorization Error: You are not allowed to update this mooring or it does not exist.' }
  }
  ...
  const { error: updateError } = await supabase
    ...
    .update(updateData)
    .eq('id', mooringId)
    .eq('owner_id', user.id) // RLS handles this, but explicit check is fine
...
//): Promise<FormState> { // Example if using useFormState
  ...
  if (fetchError || !existingMooring || existingMooring.owner_id !== user.id) {
    ...
    return { message: 'Error: You are not authorized to delete this mooring or mooring not found.' };
  }
  ...
  const { error } = await supabase
    ...
    .delete()
    .eq('id', id)
    .eq('owner_id', user.id) // RLS makes this eq redundant but adds clarity
